...
int coloredmy1(void)  // White_led on
{
   pwmSetPercent(atoi("100"),0);  // blue = on
   pwmSetPercent(atoi("100"),1);  // green = on
   pwmSetPercent(atoi("100"),2);  // red = on
} 
int coloredmy2(void)  // Blue_led on
{
   pwmSetPercent(atoi("100"),0);  // blue = on
   pwmSetPercent(atoi("0"),1);  // green = off
   pwmSetPercent(atoi("0"),2);  // red = off
} 
int coloredmy3(void)  // Green_led on
{
   pwmSetPercent(atoi("0"),0);  // blue = off
   pwmSetPercent(atoi("100"),1);  // green = on
   pwmSetPercent(atoi("0"),2);  // red = off
} 
int coloredmy4(void)  // Red_led on
{
   pwmSetPercent(atoi("0"),0);  // blue = off
   pwmSetPercent(atoi("0"),1);  // green = off
   pwmSetPercent(atoi("100"),2);  // red = on
